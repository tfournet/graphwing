...
Before any canary, re-import the integration and publish the affected graphs. Integration parity authority is the installed `$GRAPHWING_HOME/openapi.json`, not the repository's intentional loopback default: the script first fetches the public `sourceUrl` and compares it semantically with that deployed copy before any PUT. After publish it compares a fresh Rewst API readback with the same deployed authority and reports deployed/public/readback SHAs. Server destination remains semantic; unreadable or unequal operation/schema/graph bytes emit no parity receipt. Do not replace that readback with the source request body.
