...
|---|---|---|
| `graphwing-routing-policy` | `class`, `work_kind`, `size`, `ac_count`, `seams`; verified `agent_evidence` only for fallback | Build deterministic native `workflow-normal-v1` primary/reviewer decisions and one exact characterized availability alternate as complete `route-execution-profile-v2` objects. `implement-slice` calls its exact published workflow/version after daemon evidence verification. Invalid or non-availability facts hard-fail. |
