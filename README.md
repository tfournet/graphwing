...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | One bounded lifecycle decision over the same durable v2 run. Native workflow policy selects `correct`, `await_exact_head_audit`, `review_infrastructure_hold`, `verify`, or `park` from normalized PR facts and immutable run-control history. A tenant-scoped exact-head CAS claim permits at most one exactly authorized writer for a run and commit; a successful push is preserved before stopping for a fresh exact-head audit. |
