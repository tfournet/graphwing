...
The routing-policy-v2 candidate is not active; it remains benchmark-only and non-production. The workflow-owned `workflow-normal-v1` policy preserves the characterized normal-v1 writer, budget, and reviewer matrix and now supplies every primary writer and routed reviewer in `implement-slice` and `pr-drive`. Writer and reviewer effort remain independent: normal reviews request `medium`; sensitive reviews request `high`. The Python v1 endpoints and profiles remain available for rollback, fallback, and recovery compatibility; those policy branches have not moved yet. See [routing-policy-v2.md](notes/routing-policy-v2.md) for the unpromoted candidate and evidence gate.

`graphs/routing-policy.json` is the canonical source definition for policy version `workflow-normal-v1`. It uses only native object builders, a filter, hashing, and a static hard-failure node. The publisher emits it before both consumers and substitutes exact workflow/version IDs into each child call. Policy failure reaches a terminal park path before any writer or reviewer launch. This is source-catalog proof only: no deployment, re-import, publication, readback, or live invocation is claimed.
