...
| `graphwing-run-control-initialize` | `budgets`, `evaluator_contract_sha256`, `initial_route`, `root_identity`, `state_version` | Active v1 initializer called by `scripts/drive-pr.py` before `pr-drive`; it writes the closed root identity and immutable budgets through `transition`. |
| `graphwing-run-control-state` | `branch`, `callback_binding_sha256`, `candidate_route`, `endpoint`, `exact_request_body_sha256`, `handoff`, `head_sha`, `launcher_fingerprint`, `next_envelope`, `owner_workflow_run_id`, `permission_profile`, `repository`, `run_control_id`, `state_version`, `task_sha256` | Active v1 evaluator/challenge controller called by each `pr-drive` attempt; it persists at most one reservation or a terminal result. |
...
| `graphwing-run-control-consume` | `run_control_id`, `state_version` | Active v1 controller whose `authorize` node calls `consume-authorization`, verifies one outstanding reservation, and persists consumed state without launching. |
...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | One run-control-backed run, up to three in-run attempts. The exact-pinned routing-policy child supplies its v2 writer profile; each slot derives review/check actionability from normalized `pr-policy-inputs-v1` facts and sends one closed task to that route. One final persisted, head-bound named test precedes optional merge. |
