...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | One run-control-backed run, up to three in-run attempts: each slot re-reads the PR and does nothing once green. One final persisted, head-bound named test before optional merge. Start after `graphwing-run-control-initialize`. |
