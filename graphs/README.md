...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | bounded window over one durable v2 run: route only from the exact-pinned policy child, exact launch-authority preparation and one consumed signed authorization per writer, poll/wait recovery before full-envelope authority-loss reconciliation, and a continuation marker for a later execution |
