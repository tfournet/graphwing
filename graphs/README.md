...
| `graphwing-run-control-initialize` | `budgets`, `evaluator_contract_sha256`, `initial_route`, `purpose` | dormant immutable-budget initialization through an exactly pinned transition version |
| `graphwing-run-control-state` | `branch`, `callback_binding_sha256`, `candidate_route`, `endpoint`, `exact_request_body_sha256`, `handoff`, `head_sha`, `launcher_fingerprint`, `next_envelope`, `permission_profile`, `purpose`, `repository`, `task_sha256` | dormant verified-state evaluation, continuation/handoff guard, current challenge, and one persisted reservation; no launch edge |
