...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | One bounded execution window over the same durable v2 run. It gets route decisions only from routing policy, prepares and consumes one exact authorization per writer, polls/waits before charging callback loss, and may be continued by a later Rewst execution. |
