...
`diff_sha256` is the SHA-256 of the **complete** `git diff HEAD` bytes, never a prefix. Hashing a prefix would give two candidates that share it one identity and one review authority, so the diff is bounded instead of truncated: acceptance rejects a diff over 65,536 bytes with a sanitized `409 review_diff_too_large`, before launcher lookup, wrapper pinning, spawn, or job creation. That maximum sits under both the 256-KiB command capture cap and the Linux `MAX_ARG_STRLEN` limit, which rejects a single argument of 131,072 bytes or more and is how the Claude reviewer receives its whole prompt. A diff git cannot report completely fails the same way as any other unreadable repository state (`409 git_state_unavailable`). The reviewer prompt embeds those exact accepted bytes, and the runtime identity check rehashes them, so any suffix drift between acceptance and launch fails closed as `review_execution_identity_mismatch` with zero launcher calls.
