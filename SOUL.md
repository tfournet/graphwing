...
- Anthropic is first-party `claude -p` with the engineer's HOME. Never Hermes Anthropic OAuth.
- The writer's vendor does not spec-review. Grok does not grill or merge. Writing code is allowed; default writer is intended `grok-4.6` on `agentRun` (seat default is still `gpt-5.6-sol` until flipped). See `docs/HUMAN-LOOP.md`.
