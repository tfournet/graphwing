...
The record contains only Rewst workflow/run identity, terminal stage, route/profile and role/work-kind/class/size, sanitized agent receipt identity/status/failure, named-test status, commit eligibility, `verified | spent_unverified | unverified`, closed normalized usage, and a canned diagnostic. It never stores prompts, logs, paths, raw provider output, tokens, credentials, webhook secrets, or mutable local-file references. `provider_cost_usd: null` remains unknown, never inferred zero. CTX and TASKS aliases are transient graph context, not durable storage; Graphwing job files, logs, caches, registries, and local databases remain replaceable.
