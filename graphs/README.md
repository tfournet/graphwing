...
| Slug | Input | Shape |
|---|---|---|
| `graphwing-routing-policy` | `class`, `work_kind`, `size`, `ac_count`, `seams`, `agent_evidence`, `routing_run_id`, `invocation_run_id`, `route_evidence` | native normal/fallback policy plus tenant-Datastore-owned later-invocation recovery; exact route-state hashes, key/version readback, current launcher capability, and local evidence reverification gate every durable choice |
...
| `graphwing-implement-slice` | `repo`, `branch`, `index`, `ticket`, `commit_message`, `test`, `iters_left`, `class`, `work_kind`, `size`, `ac_count`, `seams`, `kick_url`, `kick_token`, `e2e`, `routing_run_id` | frontier → exact-pinned routing-policy child → v2 writer/fallback → named test → v2 opposing review → complete → commit → push; the first invocation anchors its Rewst run identity and later invocations carry only its opaque locator; every correction copies the successful session/profile and never redraws |
