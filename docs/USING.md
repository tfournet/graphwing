...
Terminal jobs and receipts expose `requested_effort`, `effective_effort`, `effort_source`, and a path-free `launcher_version` fingerprint. Session identity binds launcher/provider/model plus all four of those profile fields; resume, fallback, recovery, commit, and merge evidence rejects any drift. Every native and code-off agent record is stamped with canonical `kind: agent`; GET, wait, callback, resume, fallback, recovery, startup recovery, code-off consumption, and writer evidence require that positive classification. Missing, malformed, unknown, or known non-agent kinds are never inferred from record shape and fail closed. Graphwing hashes one opened launcher artifact, copies it into a sealed Linux descriptor, and hands that exact immutable descriptor to process creation. The fingerprint proves the exact selected wrapper artifact bytes executed; it does not attest the interpreter, PATH-resolved mise/shim, or provider package/tool bytes subsequently resolved by that wrapper. Repository provenance accepts a path-free allowlist short name (or closed internal code-off identity), a bounded Git-safe branch name, and a full lowercase 40- or 64-hex Git object ID. Branches reject leading `-` or slash, trailing/repeated slash, `..`, `@{`, `@`, control/space and Git-forbidden punctuation, dot-leading or dot-trailing components, and any component ending `.lock` case-insensitively; slash-separated names such as `feature/model-routing` remain valid. Any duplicated top-level repo, branch, or starting head must exactly match the validated session identity, and public projection uses that identity rather than the raw duplicate. Public surfaces intentionally retain the validated `starting_head` as repository provenance, but otherwise use closed typed identifiers and canned diagnostics: they exclude launcher/model-supplied SHA/PR URL values, prompts, logs, filesystem or credential paths, callback data, secrets/tokens, raw provider output, private URLs, launcher-authored summaries, and launcher-native effort tokens. Legacy interrupted jobs without a complete mutually corroborated execution profile return a schema-valid provenance failure with nullable, explicitly untrusted profile fields. Local daemon jobs, logs, caches, session files, and receipts remain replaceable corroboration; they are not durable authority. Rewst data storage is the future permanent store, and Phase 1 writes nothing to it.
