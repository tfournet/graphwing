...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | run-control-backed: route once, then a capped three-slot `logic.loop` whose each pass re-reads the PR, skips when green, otherwise reserves an attempt, launches the writer, settles from the `wait` body, reconciles, tests, commits, and pushes; the shipped final-test and merge tail runs once after `done`; start after `graphwing-run-control-initialize`; published with `--only pr-drive` behind its durable chain |
