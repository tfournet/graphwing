...
| `graphwing-run-control-initialize` | `budgets`, `evaluator_contract_sha256`, `initial_route`, `purpose` | Dormant immutable-budget initializer with non-path hashed run/state identities and an exactly pinned transition child. |
| `graphwing-run-control-state` | `branch`, `callback_binding_sha256`, `candidate_route`, `endpoint`, `exact_request_body_sha256`, `handoff`, `head_sha`, `launcher_fingerprint`, `next_envelope`, `permission_profile`, `purpose`, `repository`, `task_sha256` | Dormant verified-state evaluator/challenge controller that persists at most one reservation or a terminal result; it has no launch edge. |
