...
| `graphwing-run-control-state` | `branch`, `callback_binding_sha256`, `candidate_route`, `endpoint`, `exact_request_body_sha256`, `handoff`, `head_sha`, `launcher_fingerprint`, `next_envelope`, `permission_profile`, `repository`, `run_control_id`, `task_sha256` | active v1 child of `pr-drive`: propagated-ID state evaluation, continuation/handoff guard, current challenge, and one persisted reservation; no launch edge |
| `graphwing-run-control-reconcile` | `authority_loss_reason`, `kind`, `receipt`, `run_control_id` | active v1 child of `pr-drive`: exact receipt or full-envelope authority-loss reconciliation through `transition` |
...
| `graphwing-run-control-consume` | `run_control_id` | active v1 child of `pr-drive`: verified reservation consumption through the pinned `consume-authorization` child and durable consumed-state update; no launch edge |
...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | run-control-backed: route once, then a capped three-slot `logic.loop`; each pass re-reads bounded `pr-policy-inputs-v1` facts, uses native transforms to select exact-head PR Audit or Clean Code compatibility facts, filters/deduplicates/orders open below-bar findings and root checks, and sends one closed `pr-correction-task-v1` to the writer; PR Audit transport remains an infrastructure hold and exact-head correction binding is preserved; the final-test and merge tail runs once after `done`; start after `graphwing-run-control-initialize`; published with `--only pr-drive` behind its durable chain |
