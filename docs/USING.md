...
`diff_sha256` is the SHA-256 of the **complete** `git diff HEAD` bytes, never a prefix. Hashing a prefix would give two candidates that share it one identity and one review authority, so the diff is bounded instead of truncated: acceptance rejects a diff over 258,048 bytes (252 KiB) with a sanitized `409 review_diff_too_large`, before launcher lookup, wrapper pinning, spawn, or job creation. That maximum is derived from the host limit rather than from any one candidate's measured size, because a candidate that includes this change alters its own diff size on every commit: 252 KiB is the largest round maximum that stays clear of the 256-KiB command capture cap, leaving 4,096 bytes of headroom, so an accepted diff is always the complete bytes git produced rather than a capture-truncated prefix. It is nearly four times the superseded 64-KiB maximum, which rejected the cumulative Phase 6 candidate the complete-diff bound exists to review. The Linux `MAX_ARG_STRLEN` limit, which rejects a single argument of 131,072 bytes or more, no longer bounds the maximum: no review launcher carries the prompt in argv. Codex and Claude both receive it on the child's stdin (`claude -p` reads stdin when given no prompt argument), and Grok receives it as an ACP `session/prompt` text block. A diff git cannot report completely fails the same way as any other unreadable repository state (`409 git_state_unavailable`). The reviewer prompt embeds those exact accepted bytes, and the runtime identity check rehashes them, so any suffix drift between acceptance and launch fails closed as `review_execution_identity_mismatch` with zero launcher calls.
...
`code-off-inference-profile-v1` extends each immutable slot identity with `launcher_version`, `requested_effort`, `effective_effort`, and `inference_profile_version` alongside launcher/provider/`exact_model`. `prepare` pins every slot's requested effort from the per-work-role policy (`author` and `judge` both `default` today), normalizes the effective value through the same closed table ordinary jobs use, and hashes the exact wrapper artifact once per launcher. Each slot gets an `identity_snapshot_hashes` entry over its complete identity, and `identities_hash` covers the whole map; both are inside the immutable `manifest.json`, so its file hash and the event chain move whenever any of those values change.

An experiment whose approval is stale, whose identity is unproven, or whose effort/fingerprint cannot be pinned parks at `prepare` with an `inference_identity_unpinned:<slot>:<logical>` reason and never resolves a launcher artifact. Afterwards `candidate`, `blind`, `judge`, `aggregate`, `finalize`, and code-off `agentRun` all revalidate the manifest and return `409 codeoff_identity_unpinned` rather than running against a partially pinned identity. At launch, an effective effort or wrapper fingerprint that differs from the snapshot is `codeoff_identity_mismatch`; substitution and silent fallback are not available. Terminal receipts must agree with the snapshot on effective effort and launcher version across job, session identity, and receipt before a freeze or judge receipt is accepted.

### Aggregate economics

Finalization (including a parked terminal outcome) writes one sanitized `code-off-economics-v1` aggregate as a content-addressed artifact and records its digest in the finalized event, `final.json`, `codeOffAudit`, and the `codeOffTerminal` receipt. It reports per-slot and per-role usage for the two authors and three judges, candidate and final test counts, and the final-verification flags. Usage is funded only by an authority-sealed terminal agent receipt; anything else contributes a diagnostic (`usage_authority_unavailable`, `usage_not_reported`, …) instead of a number. Token and wall totals are lower bounds over the attempts that reported, with `missing_usage_attempts` stating how many did not, while `provider_cost_usd` and `turns_observed` are null unless every attempt in the group reported them. The aggregate carries identifiers, hashes, and counts only: no prompts, logs, paths, seeds, or blind label ordering. `codeOffAudit` now also states each judge's preference and totals in author-slot terms after finalization, so per-judge blind ordering stays unreconstructable.

Rewst owns the permanent copy. Both terminal legs of `graphwing-code-off` converge on one `logic.join.any` and persist the record with native `action.datastore.records.upsert` in tenant scope, collection `graphwing_codeoff_economics_v1`, no TTL, and the deterministic key `graphwing-codeoff-economics-v1:<WORKFLOW.runId>:code-off-terminal`. Projection, key construction, and readback comparison use native object-builder, hash, and filter nodes: no Code Expression and no procedural Nunjucks. A successful write is read back with `action.datastore.records.get` on the same scope/collection/key, and canonical JSON hashes plus key and version equality must all match. Write failure, readback failure, and readback mismatch each terminate through the proven `transforms.regexReplace` invalid-RE2 hard failure, so a lost economics record can never look like success.
