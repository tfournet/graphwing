...
Terminal jobs and receipts expose `requested_effort`, `effective_effort`, `effort_source`, and a path-free `launcher_version` fingerprint. Session identity binds effective effort and launcher version in addition to launcher/provider/model; resume, fallback, recovery, commit, and merge evidence rejects any drift. Graphwing hashes one opened launcher artifact, copies it into a sealed Linux descriptor, and hands that exact immutable descriptor to process creation. Public callback receipts contain only closed typed identifiers and canned diagnostic summaries; launcher-authored summary/SHA/PR URL fields are never copied. Authenticated agent-job reads are a closed compact projection and omit prompt, cwd, logs, callback URL/token/result, raw errors, paths, secrets, provider output, private URLs, and native tokens. Legacy interrupted jobs without a complete mutually corroborated execution profile return a schema-valid provenance failure with nullable, explicitly untrusted profile fields. Local daemon jobs, logs, caches, session files, and receipts remain replaceable corroboration; they are not durable authority. Rewst data storage is the future permanent store, and Phase 1 writes nothing to it.
