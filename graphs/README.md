...
| Slug | Input | Shape |
|---|---|---|
| `graphwing-run-control-authorize` | `record_key` | inert foundation: native durable record load/upsert/readback/canonical SHA-256 → exact found/key/version/hash gate → short-lived issued-to-consumed KV CAS; no existing launch graph calls it yet |
