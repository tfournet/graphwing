...
func TestGraphwingNativePathsAndClosedShapes(t *testing.T){
 ...
 rec:=loadGraph(t,"run-control-reconcile");rs:=graphNode(t,rec,"receipt_shape");route:=map[string]any{"route_version":"normal-v1","launcher":"codex","provider":"openai","model":"gpt-5.6-sol"};prog:=map[string]any{"checkpoint":float64(2),"failing_regression_present":true,"production_diff_bytes":float64(17),"focused_tests_green":false,"diff_fingerprint":strings.Repeat("a",64)};receipt:=map[string]any{"receipt_id":"rec1-x","attempt_id":"att1-x","run_control_id":"rc1-x","authorization_id":"rca1-x","launch_descriptor_sha256":strings.Repeat("d",64),"job_id":"job1-x","terminal_status":"failed","route":route,"usage":map[string]any{"turns":float64(1),"wall_seconds":float64(2),"tokens":float64(3),"provider_cost_usd":"0.1"},"progress":prog,"constraint_signals":[]any{},"verified_outcome":false};hash:=strings.Repeat("e",64);auth:=map[string]any{"authorization_id":"rca1-x","challenge":"challenge","consumed_kv_version":float64(2),"consumed_value_sha256":hash};res:=map[string]any{"phase":"authorization_consumed","attempt_id":"att1-x","route":route,"launch_descriptor_sha256":strings.Repeat("d",64),"launch_descriptor":map[string]any{"authorization_id":"rca1-x","server_instance_challenge":"challenge"},"authorization":auth};consumed:=map[string]any{"authorizationId":"rca1-x","serverInstanceChallenge":"challenge"};r:=map[string]any{"CTX":map[string]any{"INPUT":map[string]any{"receipt":receipt},"receipt_hash":map[string]any{"value":"x"},"receipt_canonical_hash":map[string]any{"value":"x"},"allowed_constraints":[]any{},"consumed_authorization_hash":map[string]any{"value":hash}},"TASKS":map[string]any{"load_state":map[string]any{"data":map[string]any{"run_control_id":"rc1-x","outstanding_reservation":res}},"consumed_authorization":map[string]any{"version":float64(2),"value":consumed}}}
 for _,x:=range []string{"closed","consumed","terminal_status_typed","constraints_typed","verified_outcome_typed"}{if eval(t,mapping(t,rs,x),r)!=true{t.Fatal(x)}};canon:=graphNode(t,rec,"receipt_canonical");if _,e:=Evaluate(mapping(t,canon,"value"),r);e!=nil{t.Fatal(e)}
v2:=map[string]any{"version":"route-execution-profile-v2","policy_version":"workflow-normal-v1","decision_id":"decision-186","decision_sha256":strings.Repeat("a",64),"role":"writer","work_kind":"go_coding","class":"mechanical","effective_size":"S","launcher":"codex","provider":"openai","model":"gpt-5.6-sol","requested_effort":"medium"};receipt["route"]=v2;canonical,e:=Evaluate(mapping(t,canon,"value"),r);if e!=nil{t.Fatal(e)};if !reflect.DeepEqual(canonical,receipt){t.Fatalf("v2 receipt projection changed: %#v",canonical)};raw,e:=json.Marshal(receipt);if e!=nil{t.Fatal(e)};projected,e:=json.Marshal(canonical);if e!=nil{t.Fatal(e)};if string(raw)!=string(projected)||sha256.Sum256(raw)!=sha256.Sum256(projected){t.Fatal("v2 receipt canonical/hash projection drift")};receipt["route"]=route
 for _,q:=range []struct{f string;v any}{{"receipt_id",map[string]any{}},{"job_id",[]any{}},{"progress","bad"}}{old:=receipt[q.f];receipt[q.f]=q.v;if !rejects(mapping(t,canon,"value"),r){t.Fatal("bad "+q.f)};receipt[q.f]=old};for _,q:=range []struct{f,m string;v any}{{"terminal_status","terminal_status_typed",map[string]any{}},{"verified_outcome","verified_outcome_typed","false"},{"constraint_signals","constraints_typed","bad"},{"constraint_signals","constraints_typed",[]any{1}},{"constraint_signals","constraints_typed",[]any{"unknown"}},{"constraint_signals","constraints_typed",[]any{"test_deleted","test_deleted"}}}{old:=receipt[q.f];receipt[q.f]=q.v;r["CTX"].(map[string]any)["allowed_constraints"]=[]any{};if !rejects(mapping(t,rs,q.m),r){t.Fatal("bad "+q.f)};receipt[q.f]=old};receipt["extra"]=1;r["CTX"].(map[string]any)["receipt_hash"]=map[string]any{"value":"raw"};if eval(t,mapping(t,rs,"closed"),r)!=false{t.Fatal("open receipt")};delete(receipt,"extra");r["CTX"].(map[string]any)["receipt_hash"]=map[string]any{"value":"x"}
 rc:=graphNode(t,rec,"reconcile_check");for _,x:=range []string{"consumed_authorization_matches","consumed_challenge_matches","consumed_version_matches","consumed_hash_matches"}{if eval(t,mapping(t,rc,x),r)!=true{t.Fatal(x)}};auth["challenge"]="drift";if eval(t,mapping(t,rc,"consumed_challenge_matches"),r)!=false{t.Fatal("authorization drift")};auth["challenge"]="challenge";auth["consumed_value_sha256"]=strings.Repeat("g",64);if eval(t,mapping(t,rc,"consumed_hash_matches"),r)!=false{t.Fatal("nonhex hash")};auth["consumed_value_sha256"]=strings.Repeat("0",64);if eval(t,mapping(t,rc,"consumed_hash_matches"),r)!=false{t.Fatal("wrong hash")};auth["consumed_value_sha256"]=hash
...
func TestGraphwingDormantSafetyAndExactPins(t *testing.T){
 ...
 reconcile:=loadGraph(t,"run-control-reconcile"); raw,_:=json.Marshal(graphNode(t,reconcile,"reconcile_check")); for _,field:=range []string{"consumed_authorization_matches","consumed_hash_matches","descriptor_matches","route_matches"}{if !strings.Contains(string(raw),field){t.Fatalf("receipt binding omits %s",field)}}
}
