...
|---|---|---|
| `graphwing-routing-policy` | `class`, `work_kind`, `size`, `ac_count`, `seams`, `agent_evidence`, `routing_run_id`, `invocation_run_id`, `route_evidence` | Build deterministic native `workflow-normal-v1` primary/reviewer decisions and one exact characterized availability alternate as complete `route-execution-profile-v2` objects. Tenant Datastore owns later-invocation recovery history under the opaque routing locator; current launcher capability and locally reverified terminal evidence fail closed before selection. |
...
| `graphwing-implement-slice` | `repo`, `branch`, `index`, `ticket`, `commit_message`, `test`, `iters_left`, `class`, `work_kind`, `size`, `ac_count`, `seams`, `kick_url`, `kick_token`, `e2e`, `routing_run_id` | One ticket: exact-pinned workflow route, write, named test, opposing review, complete, commit, push. The first invocation uses its Rewst run identity; later invocations carry only the opaque routing locator. Initial writers/reviewers and the single availability fallback use v2 policy profiles; corrections reuse the successful receipt identity. |
...
Code-off local state is hash-chained, content-addressed execution authority under `$GRAPHWING_HOME/codeoffs`; audit exposes the seed commitment but never the seed or blind map. Candidate/final tests use the same disposable Git-worktree recipe, and only a verified hash is safely applied. For v2, Graphwing returns sealed per-attempt and test facts while native Rewst nodes own deduplicated economics and permanent experiment/terminal records. Exact terminal readback precedes local sealing and cleanup, so local loss cannot erase the durable Rewst outcome. `approved-model-manifest-v2` makes the six exact native identities eligible, including Terra and Fable only through immutable code-off snapshots; ordinary launcher model lists are unchanged. Its bounded catalog inventory and canary evidence was obtained on 2026-08-26, not by continuous discovery: “latest” means approved latest at policy creation, and expiration parks new runs pending reviewed policy update. There is no dynamic provider discovery, OpenCode dependency, alias, substitution, fallback, or redraw. This source and its fixtures do not claim that v2 was imported, published, or run live. Git worktrees are not OS isolation.
