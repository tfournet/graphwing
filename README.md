...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | One run-control-backed run, up to three in-run attempts. Each slot derives review/check actionability from normalized `pr-policy-inputs-v1` facts with native nodes and sends a closed `pr-correction-task-v1` to the writer. One final persisted, head-bound named test precedes optional merge. Start after `graphwing-run-control-initialize`. |
