...
| `graphwing-pr-drive-loop` | `repo`, `pr_number`, `test`, `auto_merge`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | dormant in-run retry variant of pr-drive: route once, then a capped three-slot `logic.loop` whose each pass re-reads the PR, skips when green, otherwise reserves an attempt, launches the writer, settles from the `wait` body, reconciles, tests, commits, and pushes; the shipped final-test and merge tail runs once after `done`; API start only after initialize; published with `--only all` or by name behind its durable chain |
