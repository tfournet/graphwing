...
`graphs/routing-policy.json` is the canonical source definition for policy version `workflow-normal-v1`. It uses only native object builders, filters, hashing, and static hard-failure nodes. The publisher emits it before both consumers and substitutes exact workflow/version IDs into each child call. Policy failure reaches a terminal park path before any writer or reviewer launch. This is source-catalog proof only: no deployment, re-import, publication, readback, or live invocation is claimed.
...
- Primary `implement-slice` and `pr-drive` writers and routed reviews consume the exact-pinned policy child's `route-execution-profile-v2` outputs. Test-red and review-nack corrections copy launcher, provider, model, requested effort, route profile, resume job, and native session from the successful receipt and never redraw. Fallback and later-invocation recovery remain on v1 compatibility policy. This is source-catalog fixture proof only and does not claim tenant import or publication.
