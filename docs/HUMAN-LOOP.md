...
- Phase 3 carries each selected writer and reviewer effort through the Rewst graph action mappings. Initial and fallback launches read their selected route; active corrections read the successful session receipt so effort cannot drift mid-session. This is source-catalog fixture proof only and does not claim tenant import or publication.
