...
2. The exact-pinned `graphwing-routing-policy` child chooses the primary writer and routed reviewers.
...
The routing-policy-v2 candidate is not active; it remains benchmark-only and non-production. The workflow-owned `workflow-normal-v1` policy preserves the characterized normal-v1 writer, budget, reviewer, and one-hop availability-fallback matrices. It supplies every primary writer and routed reviewer in `implement-slice` and `pr-drive`, plus the one allowed initial fallback in `implement-slice`. Writer and reviewer effort remain independent: normal reviews request `medium`; sensitive reviews request `high`. The Python v1 route, fallback, and recovery endpoints remain available for rollback compatibility. Active `implement-slice` recovery is now Rewst-owned tenant Datastore policy keyed by `routing_run_id`; later invocations carry only that locator. See [routing-policy-v2.md](notes/routing-policy-v2.md) for the unpromoted candidate and evidence gate.

`agentEvidenceVerify` rereads the terminal server-authored job and returns only closed agent kind/job/role/status/failure facts, the exact pinned v2 route and session identities, repo/branch provenance, and validated lifecycle timestamps. It does not choose an alternate. Native fallback eligibility and the alternate/reviewer matrices live in `graphs/routing-policy.json`; rejection, fallback action failure, and fallback receipt failure park without another provider attempt.

`graphs/routing-policy.json` is the canonical source definition for policy version `workflow-normal-v1`. It uses native object builders, filters, switches, hashing, tenant Datastore Records, read-only Graphwing fact actions, and static hard-failure nodes. Recovery rows bind the originating Rewst run, deterministic route inputs, canonical evidence hashes, daemon-instance challenge, and primary/fallback/fresh-primary facts. Every stored local job is reverified; missing authority parks. Upsert success must survive same-key data-hash and version readback before the route returns. The publisher emits the policy before both consumers and substitutes exact workflow/version IDs into each child call. This is source-catalog proof only: no deployment, re-import, publication, tenant readback, or live invocation is claimed.

`agentRun` and `reviewRun` accept the closed launcher-neutral effort vocabulary `default | low | medium | high | max`. Phase 2 propagates the validated effective value at the shared native command boundary: Codex Sol (and code-off-only Terra) accepts all five values, with `default` effective as `high`; Claude Opus/Sonnet (and code-off-only Fable) accepts all five values and omits `--effort` only for effective `default`; Grok 4.6 accepts `default | low | medium | high`, with requested `default` effective as native `high`. Grok `max` remains unsupported. Other known values fail with `unsupported_effort`; unknown values fail with `bad_effort`. A supplied direct-call value has source `explicit`; omission records requested `default` with source `launcher_default`. Source `route` requires an exact closed v1 compatibility profile or workflow-owned `route-execution-profile-v2`. The daemon reconstructs v1 catalog choices, while v2 validation checks only the closed supported native tuple and immutable policy identity; unknown, missing, unsupported, or mismatched evidence fails before launch.
...
- `diagnostic-v1` is a compact receipt contract, not a trace dump. `provider-recovery-v1` remains request-compatible, while active later-invocation recovery reads Rewst-owned tenant Datastore state by `routing_run_id`; active corrections remain pinned.
...
- Primary `implement-slice` and `pr-drive` writers and routed reviews consume the exact-pinned policy child's `route-execution-profile-v2` outputs. Test-red and review-nack corrections copy launcher, provider, model, requested effort, route profile, resume job, and native session from the successful receipt and never redraw. `implement-slice` persists verified primary/fallback/fresh-primary routing facts in tenant Datastore and later supplies only the opaque locator. `missing_binary` recovery requires current `launcherCapability`; remote provider failures require a distinct later verified primary success. Missing local jobs, replacement-daemon challenge drift, or write/readback mismatch parks. This is source-catalog fixture proof only and does not claim tenant import or publication.
...
- Issue #188 PR 1 adds an explicit `code-off-policy-v2` initialization branch without changing the default v1 path. Rewst builds and hashes the closed participant/count/identity/effort/rubric/aggregation/classification/budget/date policy with native nodes, and its expiry/eligibility gates run before launcher resolution. Graphwing accepts no v2 seed or blind map: it privately mints/reuses the seed, pins exact slots and launcher fingerprints, creates the two author worktrees, and exposes only commitment/descriptors/hashes/path-free receipts. Exact replay is idempotent; policy drift, post-initialization expiry, or local authority loss parks with no redraw, fallback, substitution, or launch. V2 currently stops at initialization; durable v2 control, activation, publication, and live proof remain later approved work.
- Final named-test admission is bounded safety only: a known repo and recipe, canonical request, exact current head, and complete writer evidence when present. A hold, red review, draft, or check state does not stop the test admission itself; native workflow policy decides whether verification is appropriate.
- Final and post-test policy reads normalized raw PR identity, label names, review decision, mergeability/state, stable exact head, and normalized checks. It emits only `held`, `review-red`, `CI-red`, `pending`, `no-checks`, `malformed`, `unknown`, `conflicting`, `closed`, or `ready`; every non-ready pre-test state terminates before callback wait. `auto_merge` is normalized once as explicit boolean human authorization, then native nodes choose human or requested merge intent.
- PR status is remote-only and applies the same native fact policy. The merge effect still independently requires the persisted final named-test job bound to repo/PR/run/head and clean start/end, then freshly revalidates authorization, open/non-draft stable head, `MERGEABLE`/`CLEAN`, holds, review, and a nonempty terminal-green GitHub check set before one exact-head merge.
...
- Issue #188 PR 2 adds the native source-catalog durable-controller layer for explicit policy-v2 runs. Tenant Datastore Records hold a closed experiment fact and deterministic experiment-scoped transition facts; write/readback hashes, keys, and versions must match before the initialization effect. The initialization request and receipt now carry the exact workflow-generated transition ID and payload hash, and Graphwing seals that binding into its local initialization authority so a conflicting replay cannot repeat or replace the effect. No atomic Datastore create/version fence has been live-proven: mutable continuation remains disabled, transition immutability is not claimed as activated enforcement, and read-then-upsert is not CAS. Initialization authority loss is durably parked without recreating selection, judgments, or winner state. No import, publish, canary, or activation is implied.
- Issue #188 PR 5 adds source-only `code-off-aggregation-v2` ownership after the three permanent judgment readbacks. Native array filters and grouped numeric sums implement two-vote preference first, summed-score fallback, exact-tie no-winner, and the selected candidate-test pass requirement. The complete sanitized decision, evidence hashes, vote/score totals, basis, winner or null, and `promotion_requested=false` are persisted under the experiment-scoped decision key and exactly read back before a later-slice final-verification binding. Graphwing receives no aggregation or winner-confirmation request. Tie, no-winner, missing-judge, and failed-candidate-test paths cannot reach verification, promotion, commit, or push. V1 aggregation and eligibility remain intact; v2 activation, final verification, promotion, import, publication, and live proof remain separately gated.
- Issue #188 PR 6 separates v2 final verification from exact promotion. Graphwing verifies the workflow-selected frozen artifact and locked tests without touching the target; native Rewst nodes durably request promotion only after actionable facts and exact readback. The one-time promotion effect applies only the bound tree and preserves the v1 finalize path. Both v2 activation gates remain false pending separately approved live proof.
- Issue #188 PR 7 moves v2 economics and terminal outcomes into native Rewst nodes. `codeOffV2ExecutionFacts` returns only authority-verified per-attempt usage, diagnostics, test facts, receipt hashes, and timing. Rewst deduplicates authoritative job IDs, preserves missing usage, and leaves aggregate provider cost null with `provider_cost_aggregation_unavailable`. The permanent experiment row and terminal event each carry policy, full normalized judgments, decision, economics, promotion/commit/push facts, final status/reason, workflow identity, and receipt hashes; both require exact hash/key/version readback. `codeOffV2SealLocal` only drains jobs and binds disposable local authority to those durable hashes. Cleanup runs after readback and cannot erase Rewst history. V1 economics/finalization/terminalization stay compatible; v2 activation, import, publication, retention proof, and live canaries remain separately gated.
