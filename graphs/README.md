...
| `graphwing-run-control-initialize` | `budgets`, `evaluator_contract_sha256`, `initial_route`, `root_identity`, `state_version` | active v1 root called by `scripts/drive-pr.py`; closed-root-identity and immutable-budget initialization through `transition` |
| `graphwing-run-control-state` | `branch`, `callback_binding_sha256`, `candidate_route`, `endpoint`, `exact_request_body_sha256`, `handoff`, `head_sha`, `launcher_fingerprint`, `next_envelope`, `owner_workflow_run_id`, `permission_profile`, `repository`, `run_control_id`, `state_version`, `task_sha256` | active v1 child of `pr-drive`: propagated-ID state evaluation, continuation/handoff guard, current challenge, and one persisted reservation; no launch edge |
...
| `graphwing-run-control-consume` | `run_control_id`, `state_version` | active v1 child of `pr-drive`: verified reservation consumption through the pinned `consume-authorization` child and durable consumed-state update; no launch edge |
...
| `graphwing-pr-drive` | `repo`, `pr_number`, `auto_merge`, `test`, `class`, `size`, `work_kind`, `prompt`, `commit_message`, `run_control_id` | run-control-backed: exact-pinned routing-policy child once, then a capped three-slot `logic.loop`; each pass re-reads bounded `pr-policy-inputs-v1` facts and sends one closed task to the selected v2 writer profile; the compact run-control route is projected from that same policy result; the final-test and merge tail runs once after `done` |
